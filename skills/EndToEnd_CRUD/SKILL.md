...
**Pre-requisites:** **Please do read these documents to get a better understanding :** [ActionMeta-LLM.md](../ActionMeta/SKILL.md) ,[ActionModel-LLM.md](../ActionModel/SKILL.md) ,[AppActionMeta-LLM.md](../AppActionMeta/SKILL.md) ,[AppActionModel-LLM.md](../AppActionModel/SKILL.md) ,[chargebee_framework_CRUD_operation_good&bad_practices.md](../GoodPractices/SKILL.md) ,[table-end-guide](../table/complete-table-defn-workflow.md),[table-defining-guide](../table/table-defn-best-practices.md),[table-guide](../table/table-defn-starter.md),[table-guide-2](../table/table-defn-complete-guide.md).
...
More detail: embedded **§2.0** below and **[chargebee-framework-practices-LLM.md](../GoodPractices/SKILL.md)**.
...
- **`m.$roles(AppRole....)`** — authorization (**derive from RBAC**, not from unrelated doc snippets — **[AppActionMeta-LLM.md §0](../AppActionMeta/SKILL.md)**)
- **`m.$param(...)`** — request inputs (strings, longs, or **column-backed** params)
- **`m.$tab(...)`** — admin **left-nav** context (**optional** for JSON-only APIs); values must match **product IA**, not doc defaults — see **[AppActionMeta-LLM.md §0.1](../AppActionMeta/SKILL.md)**
...
Framework **`ActionMeta`** (see [ActionMeta-LLM.md](../ActionMeta/SKILL.md) is the **generic** contract: params, path, validation, txn, auth.
...
**Purpose:** One document that chains **schema → codegen → `*Action` → routes → request lifecycle**, with accurate **Ant targets** and **class roles**. Use it together with [ActionMeta-LLM.md](../ActionMeta/SKILL.md) (deep `ActionMeta` / params / paths) and [chargebee-framework-overview-LLM.md](./chargebee-framework-overview-LLM.md) (repo map).
...
Full **good/bad** list: **[chargebee-framework-practices-LLM.md §2.4](../GoodPractices/SKILL.md)**.
...
Full **good / bad** patterns, **`req.getParameter`** pitfalls, and **join vs N+1** guidance: **[chargebee-framework-practices-LLM.md §2–2.3](../GoodPractices/SKILL.md)**.
...
**Good / bad for LLMs:** see **[chargebee-framework-practices-LLM.md §2.5](../GoodPractices/SKILL.md)**.
...
7. **Security:** Set **`$roles`** from **RBAC / product spec** (least privilege) — **do not** default every action to **`CUSTOMER_SUPPORT, SALES_AGENT_CPQ`** (that pair is **demo-heavy** in docs, not a framework rule). Use **`$auth`**, site scoping (`currentSite()`, `siteId` on rows). See **[AppActionMeta-LLM.md §0](../AppActionMeta/SKILL.md)**.
8. **UI / `$tab`:** Set **`m.$tab`** only for **admin pages** — from **product IA** + similar **`cb-app`** actions; **omit** for **JSON-only** when appropriate. **Do not** paste **`MainTabs.CUSTOMERS`** on every file (**[AppActionMeta-LLM.md §0.1](../AppActionMeta/SKILL.md)**).
...
**Pair with:** [chargebee-framework-overview-LLM.md](../EndToEnd_CRUD/SKILL.md), [ActionMeta-LLM.md](../ActionMeta/SKILL.md).
...
- Use **`m.$tab(...)`** only for **UI-backed** admin pages; choose enums from **requirements** + **`$tab(`** search in the same feature — **not** automatic **`MainTabs.CUSTOMERS`** (**[AppActionMeta-LLM.md §0.1](../AppActionMeta/SKILL.md)**). Omit for **JSON-only** when siblings do.
...
- For **many rows of one table** in one POST, use **`m.$param_idx(m.$create(m.$list(m.$model(qtable))))`** (then column **`$param`**, often with **`.multi(true)`**) and **`getList(qtable)`** in **`callLogic()`** — see **§2.5** and **[action-layer §3.5](../GoodPractices/SKILL.md)**.
...
**Related:** **`§3`** (defn ↔ **`db/schema.sql`**), **[action-layer-url-endpoints-crud-LLM.md §2.4](../EndToEnd_CRUD/SKILL.md)** (same topic in the CRUD guide).
...
**Further detail:** [action-layer-url-endpoints-crud-LLM.md §3.5](../EndToEnd_CRUD/SKILL.md).
...
- Set **`m.$tab(...)`** only for **admin UI** pages, using **information architecture** + **`$tab(`** search in the same feature; **omit** for **JSON-only** APIs when appropriate (**[AppActionMeta-LLM.md §0.1](../AppActionMeta/SKILL.md)**).
